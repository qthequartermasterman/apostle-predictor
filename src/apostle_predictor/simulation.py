...
class ApostolicSimulation:
        ...
        while current_date < end_date:
            ...
            if show_monthly_composition and current_date >= next_monthly_report:
                ...
                if next_monthly_report.month == 12:
                    ...
                    next_monthly_report = date(
                        next_monthly_report.year, next_monthly_report.month + 1, 1
                    )
    ...
    def run_monte_carlo(
        ...
        for i in range(iterations):
            ...
            result = self.run_simulation(
                ...
                show_monthly_composition=show_monthly,
            )
        ...
        if apostles:
            ...
            if new_prophet.callings:
                new_prophet.callings = [
                    c
                    for c in new_prophet.callings
                    if not (
                        c.calling_type == CallingType.COUNSELOR_FIRST_PRESIDENCY
                        and c != apostle_calling
                    )
                ]
    ...
    def _display_monthly_composition(
        self, current_leaders: List[Leader], report_date: date
    ) -> None:
            ...
            for calling in leader.callings:
                if calling.status == CallingStatus.CURRENT and calling.calling_type in [
                    ...
                    if calling.calling_type == CallingType.PROPHET:
                        ...
                        sort_order = (
                            1,
                            calling.seniority or 0,
                        )  # First Presidency counselors next
                        title = "Counselor in First Presidency"
                    else:
                        sort_order = (
                            2,
                            calling.seniority or 999,
                        )  # Apostles by seniority
                        title = (
                            f"Apostle (Seniority #{calling.seniority})"
                            if calling.seniority
                            else "Apostle"
                        )
...
def run_performance_benchmark(
    leaders: List[Leader], iterations: int = 100, years: int = 10
):
    ...
    vectorized_result = vectorized_sim.run_vectorized_monte_carlo(
        leaders=leaders, years=years, iterations=iterations
    )
